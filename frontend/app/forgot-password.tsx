...
export default function ForgotPasswordScreen() {
  ...
  const handleResetPassword = async () => {
    if (!email || !password || !confirmPassword) {
      Toast.show({ type: 'error', text1: 'Perhatian', text2: 'Semua kolom wajib diisi.', position: 'top' });
      return;
    }
    if (password !== confirmPassword) {
      Toast.show({ type: 'error', text1: 'Gagal', text2: 'Konfirmasi password tidak cocok.', position: 'top' });
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        Toast.show({ type: 'success', text1: 'Berhasil', text2: 'Password Anda berhasil diperbarui!', position: 'top', visibilityTime: 3000 });
        setTimeout(() => router.replace('/login'), 1500);
      } else {
        Toast.show({ type: 'error', text1: 'Gagal', text2: response.data.message || 'Terjadi kesalahan.', position: 'top' });
      }
    } catch (error: any) {
      const msg = error.response?.data?.message || "Terjadi kesalahan koneksi ke server.";
      Toast.show({ type: 'error', text1: 'Gagal', text2: msg, position: 'top' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    // ✅ KUNCI: container putih, hijau hanya di heroSection
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        >
          {/* Hero hijau hanya setinggi kontennya */}
          ...
          {/* Card putih overlap ke hero dengan marginTop negatif */}
...
const styles = StyleSheet.create({
  // ✅ Putih — tidak ada hijau bocor di bawah
  container: { flex: 1, backgroundColor: '#fff' },

  scrollContainer: { flexGrow: 1 },

  // Hijau hanya di area hero
  heroSection: {
    backgroundColor: '#00AA5B',
    paddingHorizontal: 24,
    paddingTop: 40,
    paddingBottom: 54, // lebih besar agar overlap card terlihat mulus
  },
  ...
  // Card naik overlap ke hero, sisanya selalu putih
  formCardWrapper: {
    backgroundColor: '#fff',
